...
let rec eval_env (env : env) (e : expr) : value comp =
  ...
  |Continue -> fun _ -> raise Continue (* zachowanie typu comp udaje funckje zeby nie rzucalo wyjatkiem odrazu*)
  |Break -> fun _ -> raise Break
  | While (condition, body) -> 
      let rec loop () = 
        let* cond_val = eval_env env condition in
        match cond_val with
        | VBool true -> 
          (try
            let* _ = eval_env env body in (* zmianka heapu czyli wykonanie body *)
              loop ()
          with
          | Continue -> loop ()
          | Break -> return VUnit)
