...
let rec infer_type env (e : expr) : typ =
  ...
  | App(e1, e2) ->
    ...
    | tp -> raise (Type_error(e.pos, 
      Printf.sprintf " Expected function type, got %s"  (string_of_typ tp)))
    end
  | Fst e ->
    ...
    | tp -> 
      raise (Type_error(e.pos,
        Printf.sprintf "Expected pair type, got %s" (string_of_typ tp)))
    end
  | Snd e ->
    begin match infer_type env e with
    | TPair(_, tp2) -> tp2
...
and check_type env e tp =
  ...
  if tp = tp' then ()
    ...
    raise (Type_error(e.pos, Printf.sprintf "Type mismatch : expected %s, got %s" 
    (string_of_typ tp) (string_of_typ tp')))
