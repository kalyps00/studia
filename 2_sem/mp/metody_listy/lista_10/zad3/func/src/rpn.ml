...
let convert_to_rpn (expr : string) : prog =
  ...
  (* stack_size (convert_to_rpn "2+2*4") *) (* test *)

let stack_size (prog : prog) : int =
  let rec aux cmds current_size max_size =
    match cmds with
    | [] -> max_size
    | PushInt _ :: rest ->
        (* Dodanie liczby zwiększa stos *)
        let new_size = current_size + 1 in
        aux rest new_size (max max_size new_size)
    | Binop _ :: rest ->
        (* Operator binarny zmniejsza stos o 1 (zdejmujemy 2, dodajemy 1) *)
        let new_size = current_size - 1 in
            aux rest new_size max_size
  in
  aux prog 0 0
