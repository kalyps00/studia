...
let rec subst (x : ident) (s : expr) (e : expr) : expr =
  match e with
  |Binop (And, e1, e2) -> If (e1, e2, Bool false) (* zad1 *)
  |Binop (Or, e1, e2) -> If (e1, Bool true, e2) (* zad 1 *)
...
let rec eval (e : expr) : value =
  ...
  | Is_Unit e ->
      (match eval e with
        |VUnit -> VBool true
        |_ -> VBool false) (* zad 2 koniec *)
  | Fold (ls, x, acc, e2, e3) ->
    (let list_val = eval ls in
    let acc_start = eval e3 in
    let rec fold_right lst = 
      match lst with
      |VUnit -> acc_start
      |VPair (h, t) -> 
        let tail_result = fold_right t in
        let new_e2 = subst x (reify h) (subst acc (reify tail_result) e2) in
        eval new_e2
      | _ -> failwith "Type error"
    in
    fold_right list_val)
