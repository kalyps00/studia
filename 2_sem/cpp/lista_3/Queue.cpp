...
int Queue::length() { return num_of_elements; }
int Queue::q_capacity() { return capacity; }
// constructors
Queue::Queue(int capacity) : capacity(capacity), first(0), num_of_elements(0) {
  if (capacity <= 0) {
    throw std::invalid_argument(
        "Cannot initialize queue with non-positive cpacity");
  }
