...
void statystyki(const char *text, TextStats *stats)
{
    ...
    while (text[i] != '\0')
    {
        ...
        if (isalpha(znak))
        {
            ...
            dlugosc_slowa++;
            if ('a' <= znak && znak <= 'z')
                stats->tablica_liter[(int)znak - 97]++;
            if ('A' <= znak && znak <= 'Z')
                stats->tablica_liter[(int)znak - 65 + 26]++;
        }
        ...
        if (isspace(znak))
        {
            ...
            if (dlugosc_slowa > 0 && znak == ' ')
            {
                ...
                if (dlugosc_slowa > stats->max_dlugosc_slowa)
                    stats->max_dlugosc_slowa = dlugosc_slowa;
                if (dlugosc_slowa < stats->min_dlugosc_slowa)
                    stats->min_dlugosc_slowa = dlugosc_slowa;
...
void wyszukiwanie_wzorca(const char *text, const char *pattern)
{
    ...
    if ((pattern[0] == '.' && strlen(pattern) == 1) || (pattern[0] == '!' && strlen(pattern) == 1) || (pattern[0] == '?' && strlen(pattern) == 1) || (pattern[0] == '-' && strlen(pattern) == 1))
        wyszukiwanie_znaku_specjalnego(text, pattern);
    ...
    while (strstr(text_copy, pattern) != NULL)
    {
        ...
        int licznik = 1;
        while (buffer[strlen(buffer) - licznik] == ' ')
            licznik++;
        for (int j = strlen(buffer) - licznik; j >= 0; j--)
            printf("%c", buffer[j]);
...
void wypisz_tekst_wersja1(const char *text, int x, int pattern_len)
{
    ...
    while (buffer[strlen(buffer) - licznik] == ' ')
        licznik++;
    if (buffer[strlen(buffer) - licznik] == '-')
        licznik += 2;
    if (buffer[strlen(buffer) - licznik] == '"')
        licznik++;
    for (int i = strlen(buffer) - licznik; i >= 0; i--)
        printf("%c", buffer[i]);
    memset(buffer, 0, sizeof(buffer));
    ...
    int i;
    for (i = x + pattern_len; text[i] != '.' && text[i] != '\n' && text[i] != '\t' && text[i] != '!' && text[i] != '?' && text[i] != EOF; i++)
        printf("%c", text[i]);
    if (text[i] != '\n' && text[i] != '\t' && text[i] != EOF)
        printf("%c", text[i]);
...
void wypisz_tekst_wersja2(const char *text, int x, int pattern_len)
{
    ...
    int i, licznik = 0;
    for (i = x + pattern_len; text[i] != '\t' && text[i] != EOF && licznik <= 30; i++, licznik++)
        printf("%c", text[i]);
    if (licznik == 60)
        printf(" ...");
...
void podmiana_wzorca(const char *text, const char *pattern, const char *replacement, char *result)
{
    ...
    while (strstr(text_copy, pattern) != NULL)
    { // strstr to jakaś funkcja która zwraca wskaźnik na pierwsze wystąpienie pattern w text_copy
        text_copy = strstr(text_copy, pattern);
